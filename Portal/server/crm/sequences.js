...
async function processEnrollment(enr) {
  ...
  try {
    ...
    // Same bookkeeping as the two CRM paths: which transport carried it, and the token that
    // lets an open be recognised when there is no provider to report one.
    await query(
      ...
      [emailId, res.id, from, res.reply_to_used || null,
       res.transport === 'smtp' ? 'smtp' : 'resend', res.open_token || null]);
  } catch (e) {
