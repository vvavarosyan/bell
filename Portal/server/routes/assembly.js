  ...
  try {
    const r = await query(`
      SELECT
        ...
        (SELECT count(*)::int FROM people WHERE merge_status = 'merged_into')          AS merged_people,
        (SELECT count(*)::int FROM person_dedup_candidates WHERE decision = 'pending') AS people_pending_review
    `);
    res.json(r.rows[0]);
  } catch (err) { next(err); }
});
...
// ---------------------------------------------------------------------------
// GET /api/assembly/people/dedup-queue?limit= — pending person pairs to review
// (same name + shared employer). Includes each person's employers so the admin
// can tell them apart.
// ---------------------------------------------------------------------------
router.get('/people/dedup-queue', async (req, res, next) => {
  try {
    const limit = Math.min(Number(req.query.limit ?? 100), 300);
    const r = await query(`
      SELECT dc.id, dc.similarity_score, dc.similarity_reasons, dc.created_at,
        json_build_object(
          'id', a.id, 'pin', a.pin, 'full_name', a.full_name, 'headline', a.headline,
          'linkedin_url', a.linkedin_url, 'email', a.email,
          'source', a.extra_fields->>'source', 'scope', a.extra_fields->>'moph_scope',
          'license', a.extra_fields->>'moph_license_no',
          'companies', (SELECT array_agg(DISTINCT c.name) FROM person_companies pc JOIN companies c ON c.id=pc.company_id WHERE pc.person_id=a.id)
        ) AS person_a,
        json_build_object(
          'id', b.id, 'pin', b.pin, 'full_name', b.full_name, 'headline', b.headline,
          'linkedin_url', b.linkedin_url, 'email', b.email,
          'source', b.extra_fields->>'source', 'scope', b.extra_fields->>'moph_scope',
          'license', b.extra_fields->>'moph_license_no',
          'companies', (SELECT array_agg(DISTINCT c.name) FROM person_companies pc JOIN companies c ON c.id=pc.company_id WHERE pc.person_id=b.id)
        ) AS person_b
      FROM person_dedup_candidates dc
      JOIN people a ON a.id = dc.person_a_id
      JOIN people b ON b.id = dc.person_b_id
      WHERE dc.decision = 'pending'
      ORDER BY dc.similarity_score DESC, dc.id
      LIMIT $1`, [limit]);
    res.json({ rows: r.rows });
...
// ---------------------------------------------------------------------------
// POST /api/assembly/people/dedup/:id/decide
// body: { action: 'merge_a_to_b' | 'merge_b_to_a' | 'keep_separate', admin_email? }
// ---------------------------------------------------------------------------
router.post('/people/dedup/:id/decide', async (req, res, next) => {
  try {
    const id = Number(req.params.id);
    const { action, admin_email } = req.body || {};
    if (!['merge_a_to_b', 'merge_b_to_a', 'keep_separate'].includes(action)) {
    ...
    const r = await query(`SELECT * FROM person_dedup_candidates WHERE id = $1`, [id]);
    if (!r.rows.length) return res.status(404).json({ error: 'not_found' });
    const cand = r.rows[0];
    if (cand.decision !== 'pending') return res.status(409).json({ error: 'already_decided', decision: cand.decision });

    if (action === 'keep_separate') {
      await query(`UPDATE person_dedup_candidates SET decision='kept_separate', decided_at=now(), decided_by=$2 WHERE id=$1`,
        [id, admin_email || 'unknown']);
      return res.json({ ok: true, decision: 'kept_separate' });
    }
