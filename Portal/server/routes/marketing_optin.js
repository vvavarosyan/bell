...
router.post('/', async (req, res) => {
  try {
    ...
    // Welcome email — once per address EVER (keyed on whether a welcome was actually sent, not
    // on prior consent: subscribers from before the welcome feature existed had consent but no
    // welcome, and Val's live test hit exactly that — "already subscribed" skipped them
    // forever). Fire-and-forget: the form response never waits on the send.
    try {
      const { query } = await import('../db.js');
      const welcomed = (await query(
        `SELECT 1 FROM crm_emails WHERE sent_by='optin-welcome' AND lower(to_email)=$1 AND status IN ('sent','delivered','opened') LIMIT 1`,
        [email])).rows.length > 0;
      if (!welcomed) sendWelcome(email).catch(() => {});
    } catch { /* never block the opt-in response */ }
    res.json({ ok: true });
