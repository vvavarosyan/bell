...
router.get('/records/:id', async (req, res, next) => {
  try {
    ...
    const [notes, activities, tasks, emails, enrollments, deals] = await Promise.all([
      ...
      query(`SELECT id, direction, from_email, to_email, cc_email, subject, body_text, status,
                    error, sent_by, created_at, sent_at, opened_at, clicked_at
               FROM crm_emails WHERE record_id=$1 ORDER BY created_at DESC LIMIT 100`, [id]),
...
router.post('/records/:id/email', async (req, res, next) => {
  try {
    ...
    const ins = await query(
      ...
      [tenantId(req), id, to, cc.length ? cc.join(', ') : null, replyTo, subject, bodyOut, replyTo]);
