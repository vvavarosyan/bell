...
export const api = {
  ...
  // Discovery review queue (Maps candidates + Spark discoveries).
  discoverySummary:       () => request('/api/discovery/summary'),
  discoveryGmaps:         (limit = 100) => request('/api/discovery/gmaps?limit=' + limit),
  ...
  locTwins:               (limit = 100) => request(`/api/location-review/twins?limit=${limit}`),
  locTwinApprove:         (keep_id, drop_id) => request('/api/location-review/twin-approve', { method: 'POST', body: JSON.stringify({ keep_id, drop_id }) }),
  locTwinReject:          (a_id, b_id) => request('/api/location-review/twin-reject', { method: 'POST', body: JSON.stringify({ a_id, b_id }) }),
  chainsSummary:          () => request('/api/chains/summary'),
  chainsGroups:           (limit = 50) => request(`/api/chains/groups?limit=${limit}`),
  chainsApprove:          (parent_id, member_ids) => request('/api/chains/approve', { method: 'POST', body: JSON.stringify({ parent_id, member_ids }) }),
  chainsReject:           (parent_id, member_ids) => request('/api/chains/reject', { method: 'POST', body: JSON.stringify({ parent_id, member_ids }) }),
  ...
  // Saved lists (curated, free — switching-costs lock-in)
  lists:                  () => request('/api/crm/lists'),
  listCreate:             (name, color) => request('/api/crm/lists', { method: 'POST', body: JSON.stringify({ name, color }) }),
  listUpdate:             (id, body) => request('/api/crm/lists/' + id, { method: 'PATCH', body: JSON.stringify(body) }),
  listDelete:             (id) => request('/api/crm/lists/' + id, { method: 'DELETE' }),
  list:                   (id) => request('/api/crm/lists/' + id),
  listAddMembers:         (id, entity_ids, entity_type = 'company') => request(`/api/crm/lists/${id}/members`, { method: 'POST', body: JSON.stringify({ entity_type, entity_ids }) }),
  listRemoveMembers:      (id, entity_ids, entity_type = 'company') => request(`/api/crm/lists/${id}/members`, { method: 'DELETE', body: JSON.stringify({ entity_type, entity_ids }) }),
  listMemberships:        (entity_id, entity_type = 'company') => request('/api/crm/list-memberships?' + new URLSearchParams({ entity_id, entity_type })),
  listExportCsv:          (id, name) => downloadFile(`/api/crm/lists/${id}/export.csv`, (name || 'list') + '.csv'),
  ...
  osmContext:             (q = {}) => request('/api/osm/context?' + new URLSearchParams(q)),
  ...
  mktSendNow:             (id) => request('/api/marketing/campaigns/' + id + '/send-now', { method: 'POST' }),
  ...
  mktMail:                (direction = 'out', limit = 100, system = 'all') => request('/api/marketing/mail?direction=' + direction + '&limit=' + limit + '&system=' + system),
  mktEmailOverview:       () => request('/api/marketing/email-overview'),
  mktDigestPreview:       () => request('/api/marketing/digest/preview'),
  mktDigestSend:          () => request('/api/marketing/digest/send', { method: 'POST' }),
  mktMailOne:             (id) => request('/api/marketing/mail/' + id),
  mktMachine:             () => request('/api/marketing/machine'),
  mktResetBreaker:        () => request('/api/marketing/machine/reset-breaker', { method: 'POST' }),
  mktPreflight:           () => request('/api/marketing/machine/preflight', { method: 'POST' }),
  mktStats:               (id) => request('/api/marketing/campaigns/' + id + '/stats'),
  mktHotLeads:            () => request('/api/marketing/hot-leads'),
