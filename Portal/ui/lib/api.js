...
export const api = {
  ...
  contributions:          (params = {}) => request('/api/enrichment/contributions' + (Object.keys(params).length ? '?' + new URLSearchParams(params) : '')),
  promoteContribution:    (id) => request(`/api/enrichment/contributions/${id}/promote`, { method: 'POST', body: '{}' }),
  rejectContribution:     (id) => request(`/api/enrichment/contributions/${id}/reject`, { method: 'POST', body: '{}' }),
  newEntityProposals:     (params = {}) => request('/api/enrichment/new-entities' + (Object.keys(params).length ? '?' + new URLSearchParams(params) : '')),
  promoteNewEntity:       (id) => request(`/api/enrichment/new-entities/${id}/promote`, { method: 'POST', body: '{}' }),
  rejectNewEntity:        (id) => request(`/api/enrichment/new-entities/${id}/reject`, { method: 'POST', body: '{}' }),
