...
// The whole render is wrapped in a try/catch so a data hiccup shows a readable
// error on the page instead of blanking the app.
...
export function EngineTab() {
  ...
  if (!s) return html`<div class="dr-shell"><div class="empty">Loading engine status…</div></div>`;
  ...
  try {
    ...
    return html`
      <div class="dr-shell">
        <div class="grid-toolbar">
          ...
          <span class="muted small">Bell's own enrichment engines · local · $0 · always-on</span>
          <span class="spacer"></span>
          ${s.installed ? html`<button onClick=${toggle} disabled=${busy}>${busy ? '…' : (s.paused ? '▶ Resume' : '⏸ Pause')}</button>` : null}
          <button onClick=${load}>Refresh</button>
        </div>

        <div style=${{ padding: '16px', overflow: 'auto', maxWidth: '1000px' }}>
    ...
    return html`<div class="dr-shell"><div style=${{ padding: '24px' }}>
      ...
      <div class="muted" style=${{ marginTop: '10px' }}>Copy this message and paste it back to me — it pinpoints the exact fix.</div>
    </div></div>`;
  }
}
