...
**☀️ VAL'S MORNING LIST (2026‑07‑23) — post‑flip:** 1) ROG, 30 seconds: Device Manager → Network adapters → Wi‑Fi → Power Management → uncheck "Allow the computer to turn off this device" (the one setup leftover; the cmdlet couldn't set it). 2) Ask Claude "how did the ROG do last night?" for the first‑night report. 3) Check the 07:00 push landed (Signals → Tenders: "Local · Live ✓ synced"). 4) His deferred testing: Discovery Review (Chains 61 · Location pairs · Address twins · Maps 521 · Spark ~319) + Address Review (300) + tender "also on Kahramaa" chip. ROG ops facts: tasks under Task Scheduler \Bell\ (sweep @startup+keepalive conc 10 · nightly 00:30 · push 07:00/19:00), glue in C:\bell-ops\ outside the repo, keys.env exclude is local (a future ROG git pull picks up the tracked .gitignore from 3baa9e6). ⚠️ Flagged by ROG: nightly + continuous sweeps overlap 00:30–morning with no cross‑guard — fine on 16 GB, stagger later if wanted. Verified from the Mac 01:55: 120 companies harvested + 438 contacts in 15 min.
...
~191K companies, ~76K active, ~16K with websites. Latest migration = **112**. Deep memory: [[self-marketing-outreach]], [[data-completeness-program]], [[bellscore-and-search]].
