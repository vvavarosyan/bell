...
**📊 MEASURED, so nobody re‑derives it:** the classifieds write landed **233 vacancies in 9 s** — 111 tied to a company, 233 with a stated employer, 233 locations, 232 employment types, 212 seniority, 225 industries, 226 descriptions, all 233 with valid jsonb raw_payload; **22 closed instantly** because their own stated expiry had passed (exactly the "not misleading" behaviour). ⚠️ **The own‑site careers reader yields ZERO and that is the finding, not a bug:** across **95** of Bell's own careers pages (25 by id + 70 random), **not one publishes schema.org JobPosting**. 32 of 70 publish other structured data (WebPage · WebSite · BreadcrumbList · Organization · DiscussionForumPosting), so the tooling is there — Qatar employers just do not mark up vacancies for Google Jobs. The reader is kept (correct, free, picks up any site that adds it) but runs **weekly, not nightly** — 255 fetches a night for a measured zero is noise. **Covering every company's own site therefore needs the paid Apify actor Val already anticipated**, or guessing at page layout, which Rule 2.1 forbids.
